...
# 在计算细节上和原版GRU没有任何区别，所谓Contextual，指的是input里面部分维度的数据应当解读为Event信息
# 部分维度的数据应当被解读为描述这些Event的Contextual信息
    ...
    def __call__(self, input_x, prev_hidden_state,  task_index, time_interval):
        ...
        with tf.name_scope('CGRUHawkes_Cell'):
            ...
            with tf.name_scope('CGRU_Hawkes_Internal'):
                z_t = tf.transpose(tf.sigmoid(tf.matmul(weight_z, hawkes_intensity) + bias_z))
                r_t = tf.transpose(tf.sigmoid(tf.matmul(weight_r, hawkes_intensity) + bias_r))
